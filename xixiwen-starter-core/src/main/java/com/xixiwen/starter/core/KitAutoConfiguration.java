...
public class KitAutoConfiguration {
	...
	public CuratorFramework curatorFramework(RetryPolicy retryPolicy, KitProperties properties) throws Exception {
		...
		curator.start();
		log.info("connected to zookeeper for  blocking unit " + properties.getBlockUntilConnectedWait() + properties.getBlockUntilConnectedUnit());
		curator.blockUntilConnected(properties.getBlockUntilConnectedWait(), properties.getBlockUntilConnectedUnit());
		log.info("connected to zookeeper success");
		return curator;
	}
	...
	protected static class ZookeeperHealthConfig {
		@Bean
		@ConditionalOnMissingBean
		@ConditionalOnEnabledHealthIndicator("kit")
